...
public class GenSort {
  static final byte DELIM1 = '\n';
  static final byte DELIM2 = '\r';
  static final byte DELIM3 = (byte)0x01;
  static final byte REPLACE = (byte)0xFF;
  ...
  static void generateRecord(byte[] recBuf, Unsigned16 rand,
    ...
    // Replace all instances of DELIM with replace and set the last character
    // to delim.
    for (int i = 0; i < 100; ++i) {
      if (recBuf[i] == DELIM1 || recBuf[i] == DELIM2 || recBuf[i] == DELIM3) {
        recBuf[i] = REPLACE;
      }
    }
  }
