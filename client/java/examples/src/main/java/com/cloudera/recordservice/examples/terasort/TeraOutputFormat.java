...
public class TeraOutputFormat extends FileOutputFormat<Text,Text> {
  ...
  static class TeraRecordWriter extends RecordWriter<Text,Text> {
    ...
    public synchronized void write(Text key,
      ...
      // We want to make sure the record is always terminated with a new line and the
      // on disk size is always 100 bytes per record. They key is always 10 bytes. In
      // the case where this is used by TeraGen, value will be 90 bytes. We write the
      // first 89 followed by \n. In the other cases, where it was returned by the
      // InputFormat, it is always 89 characters, and we just write it.
      out.write(key.getBytes(), 0, key.getLength());
      if (value.getLength() == TeraInputFormat.VALUE_LENGTH) {
        out.write(value.getBytes(), 0, value.getLength() - 1);
      } else if (value.getLength() == TeraInputFormat.VALUE_LENGTH - 1) {
        out.write(value.getBytes(), 0, value.getLength());
      } else {
        throw new IOException("Unexpected record length: " + value.getLength());
      }
      out.writeByte(GenSort.DELIM1);
    }
