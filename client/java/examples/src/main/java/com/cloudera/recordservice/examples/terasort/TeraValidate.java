...
public class TeraValidate extends Configured implements Tool {
  ...
  static class ValidateMapper extends Mapper<Text,Text,Text,Text> {
    ...
    public void map(Text key, Text value, Context context)
      ...
      if (lastKey == null) {
        ...
        } else if (context.getInputSplit() instanceof RecordServiceInputSplit) {
          RecordServiceInputSplit split = (RecordServiceInputSplit)context.getInputSplit();
          filename = split.getTaskInfo().getTask().taskId.toString();
        } else {
          throw new IOException("Unknown input split type.");
        }
  ...
  private static void usage() throws IOException {
    System.err.println("teravalidate <out-dir> <report-dir> <boolean:use RecordService>");
  }
  ...
  public static void main(String[] args) throws Exception {
    int res;
    try {
      res = ToolRunner.run(new Configuration(), new TeraValidate(), args);
    } catch (Exception e) {
