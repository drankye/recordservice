...
namespace google {
// Symbolizes a program counter.  On success, returns true and write the symbol name to
// "out".  The symbol name is demangled if possible (supports symbols generated by
// GCC 3.x or newer).  Otherwise, returns false.
...
void PprofCmdLineHandler(const Webserver::WebRequest& request, stringstream* output) {
...
void PprofHeapHandler(const Webserver::WebRequest& request, stringstream* output) {
#ifdef ADDRESS_SANITIZER
  (*output) << "Heap profiling is not available with address sanitizer builds.";
#else
  Webserver::ArgumentMap::const_iterator it = request.args.find("seconds");
...
void PprofCpuProfileHandler(const Webserver::WebRequest& request, stringstream* output) {
#ifdef ADDRESS_SANITIZER
...
void PprofGrowthHandler(const Webserver::WebRequest& request, stringstream* output) {
#ifdef ADDRESS_SANITIZER
...
void PprofSymbolHandler(const Webserver::WebRequest& req, stringstream* output) {
  ...
  BOOST_FOREACH(StringPiece p, pieces) {
    ...
    if (google::Symbolize(
        reinterpret_cast<void*>(addr), symbol_buf, sizeof(symbol_buf))) {
...
void impala::AddPprofUrlCallbacks(Webserver* webserver) {
  ...
  webserver->RegisterUrlCallback("/pprof/heap",
      bind<void>(PprofHeapHandler, _1, _2));
  webserver->RegisterUrlCallback("/pprof/growth",
      bind<void>(PprofGrowthHandler, _1, _2));
  ...
  webserver->RegisterUrlCallback("/pprof/symbol",
      bind<void>(PprofSymbolHandler, _1, _2));
}
