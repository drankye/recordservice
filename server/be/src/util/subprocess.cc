...
bool OpenProcFdDir(DIR** dir) {
  *dir = opendir(kProcSelfFd);
  if (dir == NULL) {
    cerr << "OpenProcFdDir failed" << endl;
    return false;
  }
  return true;
}
...
void CloseProcFdDir(DIR* dir) {
  if (closedir(dir) == -1) {
    cerr << "Unable to close fd dir" << endl;
  }
}
...
Subprocess::~Subprocess() {
  if (state_ == kRunning) {
    cerr << "Child process " << child_pid_
         << "(" << argv_[0] << ") "
         << " was orphaned. Sending SIGKILL..." << endl;
    if (!Kill(SIGKILL)) cerr << "Failed to kill" << endl;
    int junk = 0;
    if (!Wait(&junk)) cerr << "Failed to Wait()" << endl;
  }
...
void Subprocess::SetFdShared(int stdfd, bool share) {
  assert(state_ == kNotStarted);
  assert(fd_state_[stdfd] != DISABLED);
...
static void RedirectToDevNull(int fd) {
  ...
  if (dev_null < 0) {
    cerr << "failed to open /dev/null" << endl;
  } else {
    assert(dup2(dev_null, fd));
  }
}
...
bool Subprocess::Start() {
  ...
  if (argv_.size() < 1) {
    cerr << "argv must have at least one elem" << endl;
    return false;
  }
  ...
  if (fd_state_[STDIN_FILENO] == PIPED) {
    assert(pipe2(child_stdin, O_CLOEXEC) == 0);
  }
  ...
  if (fd_state_[STDOUT_FILENO] == PIPED) {
    assert(pipe2(child_stdout, O_CLOEXEC) == 0);
  }
  ...
  if (fd_state_[STDERR_FILENO] == PIPED) {
    assert(pipe2(child_stderr, O_CLOEXEC) == 0);
  }

  DIR* fd_dir = NULL;
  if (!OpenProcFdDir(&fd_dir)) return false;
  shared_ptr<DIR> fd_dir_closer(fd_dir, CloseProcFdDir);

  int ret = fork();
  if (ret == -1) {
    cerr << "Unable to fork " << errno << endl;
    return false;
  }
...
bool Subprocess::DoWait(int* ret, int options) {
  if (state_ == kExited) {
    *ret = cached_rc_;
    return true;
  }
  ...
  if (rc == -1) {
    cerr << "Unable to wait on child " << errno << endl;
    return false;
  }
...
bool Subprocess::Kill(int signal) {
  ...
  if (kill(child_pid_, signal) != 0) {
    cerr << "Unable to kill " << errno << endl;
    return true;
  }
  return true;
}
